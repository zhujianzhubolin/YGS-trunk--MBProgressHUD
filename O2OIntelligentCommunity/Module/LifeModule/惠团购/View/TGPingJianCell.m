...
- (void)setPingJiaCellData:(NSDictionary *)data isGoods:(BOOL)isgoods{
    ...
    memberName.text = [NSString stringWithFormat:@"%@",data[@"memberName"]];
    
    tgName.text = [NSString stringWithFormat:@"%@",data[@"productName"]];
    ...
    NSString * contentInfor = nil;
    
    if (![data[@"content"] isEqual:[NSNull null]]) {
        contentInfor = [NSString stringWithFormat:@"%@",data[@"content"]];
    }else{
        contentInfor = @"";
    }
    ...
    CGSize contentH = [AppUtils sizeWithString:contentInfor font:[UIFont systemFontOfSize:13] size:CGSizeMake(IPHONE_WIDTH - 8 * 2, 20000)];
    ...
    if (imageArray.count > 0) {//重置UICollectionframe
        myCollectionView.frame = CGRectMake(10, content.frame.origin.y + contentH.height + 5, IPHONE_WIDTH - 20, 70);
