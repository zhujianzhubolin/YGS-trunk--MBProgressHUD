...
- (UITableView *)topMenuTableView
{
    if(nil == _topMenuTableView)
    {
        ...
        _topMenuTableView.layer.borderColor = [AppUtils colorWithHexString:@"eeeeea"].CGColor;
        _topMenuTableView.layer.borderWidth = 2;
        
        [self viewDidLayoutSubviewsForTableView:_topMenuTableView];
        _topMenuTableView.separatorStyle = UITableViewCellSeparatorStyleSingleLine;
...
- (UIView *)indicatorView
{
    if(nil == _indicatorView)
    {
        ...
        _indicatorView.backgroundColor = [AppUtils colorWithHexString:G_BTN_BGCOLOR];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    if(scrollView == self.contentTableView)
    {
        ...
        if (self.pageEndDeceleratingBlock) {
            UITableViewCell *cell = [self.contentTableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:self.pageIndex inSection:0]];
            self.pageEndDeceleratingBlock(cell,self.pageIndex);
        }
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if(tableView == self.topMenuTableView)
    {
        ...
        if (self.itemClickBlock) {
            UITableViewCell *cell = [self.topMenuTableView cellForRowAtIndexPath:indexPath];
            self.itemClickBlock(cell,indexPath.row);
        }
    }
}
